...
@Component({
  selector: 'app-test-epreuves',
  standalone: false,
  
  templateUrl: './test-epreuves.component.html',
  styleUrl: './test-epreuves.component.css'
})
export class TestEpreuvesComponent implements OnInit{
analyseId: number = 0; 
  ...
  retourAnalyses() {
    ...
    if (laboratoireId) {
      this.router.navigate([`/analyse/${laboratoireId}`]);
    } else {
